...
<form action="$self" method="post">
 <div>
 <p><input type="hidden" name="plugin" value="md5" />
  <label for="_p_md5_phrase">Phrase:</label>
  <input type="text" name="phrase"  id="_p_md5_phrase" size="60" $value/>
	</p>
EOD;

	if ($sha1_enabled) $form .= <<<EOD
  <p><input type="radio" name="scheme" id="_p_md5_sha1" value="x-php-sha1" />
...
EOD;
  ...
  <label for="_p_md5_lcrpt">LDAP CRYPT *</label></p>

  <p><input type="checkbox" name="prefix" id="_p_md5_prefix" checked="checked" />
  <label for="_p_md5_prefix">Add scheme prefix (RFC2307, Using LDAP as NIS)</label></p>

  <p><label for="_p_md5_salt">Salt, '{scheme}', '{scheme}salt', or userPassword itself to specify:</label><br />
  <input type="text" name="salt" id="_p_md5_salt" size="60" /></p>
