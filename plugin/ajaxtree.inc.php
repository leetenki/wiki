...
if (!defined('PLUGIN_AJAXTREE_HIDE_TOPLEVEL_LEAVES')) {
    define('PLUGIN_AJAXTREE_HIDE_TOPLEVEL_LEAVES', false);// kazuwaya
}
...
if (!defined('PLUGIN_AJAXTREE_NON_LIST')) {
    define('PLUGIN_AJAXTREE_NON_LIST', 'BracketName|FormattingRules|InterWiki|InterWikiName|InterWikiSandBox|MenuBar|PHP|PukiWiki|RecentChanges|RecentDeleted|SandBox|WikiEngines|WikiName|WikiWikiWeb|YukiWiki|ヘルプ');// kazuwaya
}
...
function plugin_ajaxtree_init()
{
    $messages['_ajaxtree_messages'] = array(
        'title'   => 'サイトナビ',// kazuwaya
        'toppage' => 'ホーム'// kazuwaya
    );
...
//    $javascript = 'common/js/ajaxtree.js';

    $html = '<div id="ajaxtree">' . "\n"
          . '<h2>' . htmlspecialchars($_ajaxtree_messages['title']) . '</h2>' . "\n"
...
function plugin_ajaxtree_get_cachename($page)
{
    ...
    return PLUGIN_AJAXTREE_HTML_DIR . rawurlencode($page) . '.html';
}
...
function plugin_ajaxtree_get_root_html()
{
    ...
    if (PLUGIN_AJAXTREE_TOP_DEFAULTPAGE) {
        ...
        $html    = '<dl><dt><a title="' . $title . '" href="' . $url . '">' . $s_label . $count . '</a></dt>' . "\n";
    }

    $html .= '<dd><ul>' . plugin_ajaxtree_get_subtree('/') . '</ul></dd></dl>';
