...
// 更新履歴
// 09-01-12 newpage_subdir.inc.phpに対応
// 09-11-16 入力支援に対応するためにtextareaにidを付与
// 09-11-20 content-typeの条件分岐を修正
	
// Show page-content
function catbody($title, $page, $body)
{
	global $script, $vars, $arg, $defaultpage, $whatsnew, $about_page, $help_page, $hr;// kazuwaya
	...
	// Set $_LINK for skin
	$_LINK['about']    = "$script?" . rawurlencode($about_page); // kazuwaya
	...
	$_LINK['newsub']   = "$script?plugin=newpage_subdir&amp;directory=$r_page/"; // kazuwaya
	...
	$link_about     = & $_LINK['about']; // kazuwaya
	$link_add       = & $_LINK['add'];
	$link_new       = & $_LINK['new'];	// New!
	$link_newsub    = & $_LINK['newsub']; // kazuwaya
	...
	if ($search_word_color && isset($vars['word'])) {
		$body = '<p>' . $_msg_word . htmlspecialchars($vars['word']) . // kazuwaya
			'</p>' . $hr . "\n" . $body; // kazuwaya
...
$template
  ...
  <textarea name="msg" rows="$rows" cols="$cols" id="editor">$s_postdata</textarea>
  <br />
...
function pkwk_output_dtd($pkwk_dtd = PKWK_DTD_XHTML_1_1, $charset = CONTENT_CHARSET)
{
	...
	if ($pkwk_dtd == PKWK_DTD_XHTML_1_1) {
